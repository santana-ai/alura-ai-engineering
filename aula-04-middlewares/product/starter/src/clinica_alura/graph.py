...
from clinica_alura.guardrails import APPROVAL_POLICY, PatientAccessGuardrail, detect_cpf, retry_tool
...
def assistant(state: TriageState, config: RunnableConfig, runtime: Runtime[ClinicContext]) -> dict:
    ...
    # )
    # # TODO: monte a lista de middleware do agente (PII de email, PII de CPF com detect_cpf,
    # # PatientAccessGuardrail, retry_tool, HumanInTheLoopMiddleware com APPROVAL_POLICY)
    # agent = create_agent(
    #     model=model,
    #     tools=TOOLS,
    ...
    # # usa a versão pós-middleware da mensagem (ex.: com PII já redigido), não a capturada antes do invoke
    # redacted_human_message = next(
    #     (m for m in result["messages"] if getattr(m, "id", None) == human_message.id), human_message
    # )
    # return {"messages": result["messages"], "transcript": [redacted_human_message, final_answer]}
